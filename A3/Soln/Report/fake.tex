...
\begin{document}
	...
	\begin{homeworkProblem}
		...
		\begin{enumerate}
			...
			The best performing depth was chosen to be {} because it has the highest validation performance.
	...
	\begin{homeworkProblem}
		...
		\begin{enumerate}
			\item 
			The first split of the tree is on the word $w$: ``the". Through the following calculations, we get that:
			
			\begin{align*}
			H(w | Y = real) &= -P(w) \log(P(word | Y = real)) + -P(not\ word) \log(P(not\ word | Y = real))\\
			H(w | Y = fake) &= -P(w) \log(P(word | Y = fake)) + -P(not\ word) \log(P(not\ word | Y = fake))\\
			H(w | Y) &= P(Y = real) H(w | Y = real) + P(Y = fake) H(w | Y = fake)\\
			H(Y) &= -P(Y = real) \log (P(Y = real)) + -P(Y = fake) \log (P(Y = fake))\\
			I(Y; w) &= H(Y) - H(w | Y)\\
			\end{align*}
